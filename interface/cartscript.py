def is_valid(coord_points):
    """returns True if all points are in a unique bin, False otherwise"""
...
def shunt_point(coord):
    ...
    for point in [
        (min(x + 1, num_x_grid), y),
        (min(x + 1, num_x_grid), min(y + 1, num_y_grid)),
        (min(x + 1, num_x_grid), max(0, y - 1)),
        (max(0, x - 1), y),
        (max(0, x - 1), min(y + 1, num_y_grid)),
        (max(0, x - 1), max(0, y - 1)),
        (x, min(y + 1, num_y_grid)),
        (x, max(0, y - 1)),
    ]:
    ...
    prop_x_plus_empty = (
        ...
        * len(
            [
                ...
                if len(coord_points.get(str(i) + "_%d" % y, [])) == 0
            ]
        )
        / (num_x_grid - x)
        if x != num_x_grid
    ...
    prop_y_plus_empty = (
        ...
        * len(
            [
                i
                for i in range(y + 1, 41)
                if len(coord_points.get("%d_" % x + str(i), [])) == 0
            ]
        )
        / (num_y_grid - y)
        if y != num_y_grid
    ...
    delete_old_point(x, y, ac_to_shunt)
    if prop_x_plus_empty is max(
        prop_x_plus_empty, prop_x_minus_empty, prop_y_plus_empty, prop_y_minus_empty
    ):
